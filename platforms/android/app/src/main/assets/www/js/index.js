...
const app = {
    ...
    resolveLocalFileSystemURL(
      ...
      entry => {
        ...
        entry.copyTo(
          ...
          fileName,
          permFile => {
            console.log("permfile is ......")
            console.log(permFile)
            app.permImgURI = permFile.nativeURL;
            console.log(app.permImgURI)
            app.permFile = permFile;

            console.log("here is : !!!! ", app.permImgURI);
  ...
  getReviews: () => {
    if (localStorage.getItem(app.KEY)) {
      ...
      app.reviews = JSON.parse(str); //convert string to JS object
    }
  },

  addListeners: () => {
    //from Home to Add || load camera
  ...
  findImg: (ev) => {
    //getAttribute on DetailsPage里的img
  ...
  takephotos: (ev) => {
    ...
    app.nav(ev); //take photo then go AddPage
  },
